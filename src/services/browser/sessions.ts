...
function getSession(id: string): Session {
  ...
  if (!session) throw new Error("Session browser introuvable ou expirée.");
...
export async function openBrowserSession(
  ...
  assertPublicUrl(url);
  cleanupExpired();
  if (sessions.size >= MAX_SESSIONS) throw new Error("Trop de sessions browser simultanées.");
...
export async function clickBrowserSession(sessionId: string, selector: string): Promise<void> {
  ...
  await page.evaluate<void>(`(() => { const selector = ${JSON.stringify(selector)}; const el = document.querySelector(selector); if (!el) throw new Error("Élément introuvable: " + selector); el.click(); })()`);
}

export async function fillBrowserSession(sessionId: string, selector: string, value: string): Promise<void> {
  const { page } = getSession(sessionId);
  await page.evaluate<void>(`(() => { const el = document.querySelector(${JSON.stringify(selector)}); if (!el) throw new Error("Élément introuvable"); el.focus(); el.value = ${JSON.stringify(value)}; el.dispatchEvent(new Event("input", { bubbles: true })); el.dispatchEvent(new Event("change", { bubbles: true })); })()`);
}
...
export async function textBrowserSession(sessionId: string, selector?: string): Promise<string> {
  ...
  return page.evaluate<string>(selector
    ? `(() => { const el = document.querySelector(${JSON.stringify(selector)}); if (!el) throw new Error("Élément introuvable"); return el.innerText ?? el.textContent ?? ""; })()`
...
export async function htmlBrowserSession(sessionId: string, selector?: string): Promise<string> {
  const { page } = getSession(sessionId);
  return page.evaluate<string>(selector
    ? `(() => { const el = document.querySelector(${JSON.stringify(selector)}); if (!el) throw new Error("Élément introuvable"); return el.outerHTML; })()`
